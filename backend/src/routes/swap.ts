...
///Import neccessary package for calling swap contract
import {
  Keypair,
  Connection,
  PublicKey,
...
import {TokenSwap, CurveType,TOKEN_SWAP_PROGRAM_ID} from '../utils';
import {newAccountWithLamports} from '../utils/util/new-account-with-lamport';
import {sleep} from '../utils/util/sleep'

dotenv.config();
const router = Router();

///get config datas from env file
const AutopublicKey = process.env.AutopublicKey;
const DcapublicKey = process.env.DcapublicKey;
const SpampublicKey = process.env.SpampublicKey;
// const swapid=process.env.swapid;
// const TOKEN_SWAP_PROGRAM_ID = new PublicKey(`${swapid}`);

router.get('/api/swap', async (req: Request, res: Response) => {
